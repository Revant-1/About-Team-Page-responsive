...
function About() {
  ...
  {Array(14)
    ...
    .map((_, index) => (
      <div
        className={`imgBx ${activeContent === `content${index + 1}` ? 'active' : ''}`}
        style={{ '--i': (360 / 14) * index }}
        data-id={`content${index + 1}`}
        key={index} // Make sure to add a unique key for each mapped element
      >
        <img src={contentData[index].img} alt="" /> {/* Assuming you have an array of contentData */}
      </div>
    ))}
</div>
<div className="content">
      {contentData.map((content, index) => (
        <div
          className={`contentBx ${activeContent === `content${index + 1}` ? 'active' : ''}`}
          id={`content${index + 1}`}
          key={index}
        >
          <Card
